...
class PoseYOLOv5l6(nn.Module):
    def __init__(self, heads, config_file):
        self.heads = heads
        super(PoseYOLOv5l6, self).__init__()
        self.backbone = Model(config_file)
        for head in sorted(self.heads):
            num_output = self.heads[head]
            fc = nn.Sequential(
...
class PoseYOLOv5x6(nn.Module):
    def __init__(self, heads, config_file):
        self.heads = heads
        super(PoseYOLOv5x6, self).__init__()
        self.backbone = Model(config_file)
        for head in sorted(self.heads):
            num_output = self.heads[head]
            fc = nn.Sequential(
...
def get_pose_net_yolov5s(num_layers, heads, head_conv):
...
def get_pose_net_yolov5l6(num_layers, heads, head_conv):
    config_file = os.path.join(
        os.path.dirname(__file__),
        'networks/config/yolov5l6.yaml'
    )
    pretrained = os.path.join(
        os.path.dirname(__file__),
        '../../../models/yolov5l6.pt'
    )
    model = PoseYOLOv5l6(heads, config_file)
    initialize_weights(model, pretrained)
    return model

def get_pose_net_yolov5x6(num_layers, heads, head_conv):
    config_file = os.path.join(
        os.path.dirname(__file__),
        'networks/config/yolov5x6.yaml'
    )
    pretrained = os.path.join(
        os.path.dirname(__file__),
        '../../../models/yolov5x6.pt'
    )
    model = PoseYOLOv5x6(heads, config_file)
    initialize_weights(model, pretrained)
    return model
